...
Open `my_data.csv` in Excel, Numbers, or any text editor and fill in the **Your data** column. See [my_data.csv reference](my_data_reference.md) for a field-by-field explanation.
...
| File | What it is |
|---|---|
| `busy_therapists.csv` | Therapists contacted for general outreach (options 1 & 3) |
| `probationary_therapists.csv` | Therapists contacted for probationary sessions (option 2) |
| `private_therapists.csv` | Private therapists contacted for Kostenerstattung (option 4) |
| `responses.csv` | Your contact log — fill this in manually as replies come in |
...
| `therapists.txt` | Human-readable list of therapists found in the last run |
...
**Fresh start:** Delete the relevant CSV from `output/` before running. The next run will create a new one.
